<x-app-layout>
    ...
    <div class="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div class="flex items-center gap-2 w-full sm:w-auto">
            <a href="{{ route('attendance.report.pdf', $course->id) }}" class="flex-1 sm:flex-none bg-red-600 text-white px-4 py-2 rounded-xl text-xs font-bold hover:bg-red-700 transition flex items-center justify-center shadow-sm gap-2">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                Export PDF
            </a>

            <a href="{{ route('attendance.report.excel', $course->id) }}" class="flex-1 sm:flex-none bg-emerald-600 text-white px-4 py-2 rounded-xl text-xs font-bold hover:bg-emerald-700 transition flex items-center justify-center shadow-sm gap-2">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                Export Excel
            </a>

            <button onclick="window.print()" class="flex-1 sm:flex-none bg-slate-800 text-white px-4 py-2 rounded-xl text-xs font-bold hover:bg-slate-900 transition flex items-center justify-center shadow-sm gap-2">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 00-2 2h2m2 4h10a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2z"></path></svg>
                Cetak
            </button>
        </div>
    </div>
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="hover:bg-gray-50/50 transition">
                        ...
                        @foreach($meetings as $meeting)
                            @php
                                $code = $data->per_meeting_status[$meeting->id] ?? '0';
                            ...
                            <td class="px-2 py-4 text-center">
                                <span class="w-6 h-6 inline-flex items-center justify-center rounded-md text-[10px] font-black {{ $badgeClass }}">
                                    {{ $code }}
                                </span>
                            </td>
                        @endforeach
                        ...
                        <td class="px-4 py-4 text-center text-sm font-bold text-blue-600 bg-gray-50/30">{{ $data->sakit }}</td>
