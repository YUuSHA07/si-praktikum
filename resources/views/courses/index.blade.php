<x-app-layout>
    ...
    {{-- Form Enroll Khusus Mahasiswa --}}
    @if(auth()->user()->role === 'Mahasiswa')
    <div class="mb-8 bg-indigo-900 rounded-xl p-6 text-white shadow-lg">
        <div class="md:flex items-center justify-between">
            <div>
                <h3 class="text-xl font-bold">Gabung Kelas Praktikum</h3>
                <p class="text-indigo-200 text-sm">Masukkan kode yang diberikan oleh Dosen/Aslab.</p>
            </div>
            <form action="{{ route('courses.enroll') }}" method="POST" class="mt-4 md:mt-0 flex gap-2">
                @csrf
                <input type="text" name="enrollment_code" 
                    class="rounded-lg border-none text-gray-900 focus:ring-2 focus:ring-indigo-400 font-mono uppercase" 
                    placeholder="KODE67" required>
    ...
    <div class="mb-6 flex justify-between items-center">
        ...
        @if(auth()->user()->role === 'Laboran' || auth()->user()->role === 'Dosen')
